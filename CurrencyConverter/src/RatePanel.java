// ******************************************************************
// RatePanel.java
//
// Panel for a program that converts different currencies to
// U.S. Dollars
// ******************************************************************
import java.awt.*;
import java.awt.event.*;
import javax.swing.*;
public class RatePanel extends JPanel
{
    private double[] rate; // exchange rates
    private String[] currencyName;
    private JLabel result, convert, convert_result;
    private JComboBox currency;
    private JTextField user_cost;
    // ------------------------------------------------------------
    // Sets up a panel to convert cost from one of 6 currencies
    // into U.S. Dollars. The panel contains a heading, a text
    // field for the cost of the item, a combo box for selecting
    // the currency, and a label to display the result.
    // ------------------------------------------------------------
    public RatePanel ()
    {
        JLabel title = new JLabel ("How much is that in dollars?");
        title.setAlignmentX (Component.CENTER_ALIGNMENT);
        title.setFont (new Font ("Helvetica", Font.BOLD, 20));
        // Set up the arrays for the currency conversions
        currencyName = new String[] {"Select the currency..",
        "European Euro", "Canadian Dollar",
        "Japanese Yen", "Australian Dollar",
        "Indian Rupee", "Mexican Peso"};
        
        currency = new JComboBox(currencyName);
        
        rate = new double [] {0.0, 1.2103, 0.7351,
        0.0091, 0.6969,
        0.0222, 0.0880};
        result = new JLabel (" ------------ ");
        add (title);
        add (currency);
        currency.addActionListener(new ComboListener());
        add (result);
        
        convert = new JLabel ("Enter the cost you want to convert:");
        convert.setAlignmentX (Component.BOTTOM_ALIGNMENT);
        convert.setFont (new Font ("Helvetica", Font.BOLD, 20)); 
        convert_result = new JLabel ();
        user_cost = new JTextField(20);
        add (convert);
        add (user_cost);
        
        JButton conBtn = new JButton("Convert");
        conBtn.addActionListener(new ConvertListener());
        user_cost.addActionListener(new ConvertListener());
        add (conBtn);
        add (convert_result);
        
    }
    // ******************************************************
    // Represents an action listener for the combo box.
    // ******************************************************
    private class ComboListener implements ActionListener
    {
        // --------------------------------------------------
        // Determines which currency has been selected and
        // the value in that currency then computes and
        // displays the value in U.S. Dollars.
        // --------------------------------------------------
        public void actionPerformed (ActionEvent event)
        {
            int index = currency.getSelectedIndex();
            if(index != 0) {
                result.setText ("1 " + currencyName[index] +
                " = " + rate[index] + " U.S. Dollars");                
            }
        }
    }
    
    private class ConvertListener implements ActionListener
    {
        // --------------------------------------------------
        // Determines which currency has been selected and
        // the value in that currency then computes and
        // displays the value in U.S. Dollars.
        // --------------------------------------------------
        public void actionPerformed (ActionEvent event) throws NumberFormatException
        {
            double cost;
            int index = currency.getSelectedIndex();
            if(index != 0){
                try {
                    cost = Double.valueOf(user_cost.getText());
                    convert_result.setText (cost + " " + currencyName[index] +
                    " = " + (rate[index]* cost) + " U.S. Dollars");
                } catch(Exception e) {
                    convert_result.setText("Please input a number.");
                }                
            } else {
                convert_result.setText("Please select currency name.");
            }
        }
    }
}